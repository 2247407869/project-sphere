...
class GraphitiWrapper:
        ...
        try:
            if self.graphiti:
                ...
                for result in results:
                    # 提取评分：尝试多个可能存在评分的属性名
                    score = 1.0
                    for attr in ['score', 'similarity', 'rank', 'certainty']:
                        if hasattr(result, attr):
                            score = float(getattr(result, attr, 1.0))
                            break
                ...
                logger.info(f"✅ 搜索完成，返回前 {len(formatted_results)} 个结果")
    ...
    async def get_episodes(self, limit: int = 100) -> List[Dict[str, Any]]:
        ...
        try:
            if self.graphiti:
                # 仅使用官方 retrieve_episodes 接口，不再添加 Cypher 兜底
